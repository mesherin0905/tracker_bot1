...
def should_notify(fill: dict) -> bool:
    try:
        ...
        volume = size * price
        if f["coins"] and coin not in f["coins"]:
            return False
        if volume < f["min_volume_usd"]:
            return False
        if pnl == 0 and not f["notify_opening"]:
            return False
        if pnl != 0 and not f["notify_closing"]:
            return False
        return True
    except Exception as e:
        log.error(f"Ошибка фильтра: {e}")
        return True

# ==================== HYPERLIQUID API ========================
async def hl_request(payload: dict) -> dict:
    global http_session
    try:
        if http_session is None or http_session.closed:
            log.warning("Пересоздаю HTTP сессию...")
        ...
        async with http_session.post(
            ...
            log.error(f"HL API ошибка {resp.status}")
    except asyncio.TimeoutError:
        log.error("Таймаут запроса к HL API")
    except aiohttp.ClientError as e:
        log.error(f"Ошибка соединения: {e}")
    except Exception as e:
        log.error(f"Неожиданная ошибка API: {e}")
    return {}

async def get_fills(address: str) -> list:
    data = await hl_request({
        "type": "userFills",
        "user": address.lower(),
        "limit": 50
    })
...
async def get_positions(address: str) -> list:
    ...
    if isinstance(data, dict):
        return [
            p["position"] for p in data.get("assetPositions", [])
...
def format_fill_msg(fill: dict, address: str, label: str = "") -> str:
    try:
        ...
        wallet_display = (
            f"{label} (<code>{short_addr(address)}</code>)"
            if label else f"<code>{short_addr(address)}</code>"
        )
        pnl_line = ""
        if pnl != 0:
            emoji = "💚" if pnl >= 0 else "💔"
        ...
        log.error(f"Ошибка форматирования fill: {e}")
        return "⚠️ Новая сделка (ошибка форматирования)"
...
def format_position_msg(pos: dict, address: str, label: str = "") -> str:
    try:
        ...
        return (
            ...
            f"💵 Маржа ${margin:,.2f} | "
            f"{pnl_emoji} PnL {pnl:+.2f}$ ({pnl_pct:+.1f}%){liq_str}"
        )
    except Exception as e:
        log.error(f"Ошибка форматирования позиции: {e}")
        return "⚠️ Ошибка отображения позиции"
    ...
    try:
        ...
        if addr not in last_known_fills:
            last_known_fills[addr] = {
                f"{f.get('time')}_{f.get('coin')}_{f.get('sz')}_{f.get('px')}"
                for f in fills
            }
        ...
        for fid, fill in new_fills[:10]:
            try:
                ...
                log.info(f"Уведомление отправлено: {addr[:10]} {fill.get('coin')}")
                known.add(fid)
        ...
        log.error(f"Критическая ошибка check_address {addr[:10]}: {e}")
...
async def monitoring_loop(bot: Bot):
    ...
    while bot_running:
        try:
            ...
            log.error(f"Ошибка в monitoring_loop: {e}")
...
async def cmd_list(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    for addr, _ in chunk:
        keyboard.append([
            InlineKeyboardButton(f"❌ Удалить {short_addr(addr)}", callback_data=f"del_{addr}")
        ])
    nav = []
    if page > 0:
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for addr, label in wallets.items():
        ...
        text += (
            ...
            f"  Лучшая: +${s.get('best_trade', 0):,.2f} | "
            f"Худшая: ${s.get('worst_trade', 0):,.2f}\n\n"
        )
...
async def keyboard_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📋 Кошельки":
        ...
        st = "приостановлен ⏸" if monitoring_paused else "активен ▶️"
        await update.message.reply_text(
            f"Мониторинг {st}",
            reply_markup=main_keyboard()
        )
        ...
        await update.message.reply_text("🔄 Обновляю...", reply_markup=main_keyboard())
        await cmd_status(update, context)
